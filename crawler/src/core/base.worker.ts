...
export abstract class BaseWorker {
  ...
  protected toSafeRecord(candidate: IVideoRecordCandidate): IYouTubeVideoRaw | null {
    ...
    if (this.isOlderThanLimit(postDate, fetchedAt)) {
      return null;
    }

    const url = String(candidate.url ?? `https://www.youtube.com/shorts/${id}`).trim();
  ...
  private isOlderThanLimit(postDate: string, fetchedAt: string): boolean {
    ...
    if (!Number.isFinite(postTime) || !Number.isFinite(fetchTime)) {
      return false;
    }
