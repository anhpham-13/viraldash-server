...
const INPUT_FILE = "data/youtube/viral_vids_yt.jsonl";
const OUTPUT_FILE = "data/youtube/hashtag_yt.json";

const MAX_HASHTAGS = 80;
const MIN_OCCURRENCES = 2;
const ONLY_SHORTS = true;
...
export async function runExtractHashtags(): Promise<void> {
  ...
  for (const video of videos) {
    ...
    const viewCount = safeNumber(video.statistics?.viewCount);
    const likeCount = safeNumber(video.statistics?.likeCount);
    const commentCount = safeNumber(video.statistics?.commentCount);
    const viralScore =
  ...
  // Sync to MongoDB so the frontend can query hashtags by platform + 48h window.
  try {
