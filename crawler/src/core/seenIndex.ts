  ...
  return {
    ...
    async buildFromExistingFiles(filePaths: string[]): Promise<void> {
      ...
      if (existsSync(markerPath)) {
        console.log(`[seenIndex] Seen index exists at ${indexDir}; skipping rebuild.`);
        return;
      }

      console.log(`[seenIndex] Building file-backed seen index at ${indexDir}. This is one-time and streaming-safe.`);
      ...
      try {
        for (const filePath of filePaths) {
          ...
          console.log(`[seenIndex] Indexing IDs from ${filePath}`);
          const rl = createInterface({ input: createReadStream(filePath, { encoding: "utf8" }), crlfDelay: Infinity });
      ...
      console.log(`[seenIndex] Seen index built. Indexed approx ${totalIndexed} IDs.`);
    },
  };
}
